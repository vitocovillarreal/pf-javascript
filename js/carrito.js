...
function mostrarProductosCarrito() {
    ...
    if (productosEnCarrito && productosEnCarrito.length > 0) {
        ...
        productosEnCarrito.forEach(producto => {
            ...
            div.classList.add("carrito-indu");
            div.innerHTML = `
                <img class="carrito-indu-img" src="${producto.imagen}" alt="${producto.nombre}">
                <div class="carrito-indu-titulo">
                    <small>Nombre</small>
                    <p class="carrito-indu-detalle">${producto.nombre}</p>
                </div>
                <div class="carrito-indu-cantidad">
                    <small>Cantidad</small>
                    <p>${producto.cantidad}</p>
                </div>
                <div class="carrito-indu-precio">
                    <small>Precio</small>
                    <p>$${producto.precio}</p>
                </div>
                <div class="carrito-indu-subtotal">
                    <small>Subtotal</small>
                    <p>$${producto.precio * producto.cantidad}</p>
                </div>
                <button class="carrito-indu-eliminar" id=${producto.id}><i class="fa-sharp fa-solid fa-trash"></i></button>
            `;
